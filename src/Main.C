...
typedef std::string (*sendOrderType) (CURL *curl, Parameters params, std::string direction, double quantity, double price);
typedef bool (*isOrderCompleteType) (CURL *curl, Parameters params, std::string orderId);
...
/*    json_error_t error;
    ...
    sendOrderType sendOrder[] = {Bitfinex::sendOrder, OkCoin::sendOrder, Bitstamp::sendOrder, Kraken::sendOrder};
    isOrderCompleteType isOrderComplete[] = {Bitfinex::isOrderComplete, OkCoin::isOrderComplete, Bitstamp::isOrderComplete, Kraken::isOrderComplete};
    ...
    params.addExchange("Bitfinex", 0.0000, true);
    params.addExchange("OKCoin", 0.0000, false);
    params.addExchange("Bitstamp", 0.0000, false);
    params.addExchange("Kraken", 0.0000, true);
                ...
                for (unsigned j = 0; j < params.nbExch(); ++j) {
                    if (i != j) {
                        if (checkEntry(btcVec[i], btcVec[j], res, params)) {
                            ...
                            std::string longOrderId = "";
                            std::string shortOrderId = "";
